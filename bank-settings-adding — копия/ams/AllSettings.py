...
from config import EMAIL, PASSWORD, CUSTOMER, RESTAURANT, API_PROXY_PASSWORD, M4, BACKOFFICE_URL


def is_device_tablet(device_name_param):
    NON_TABLET_PREFIXES = ('UBI', 'NUC', 'КС-4', 'KC-4 ', 'V365', 'SHOKO', 'KC-4 ', 'Ubi S/N')
    return not device_name_param.startswith(NON_TABLET_PREFIXES)


def is_device_have_bank_settings(device_bank_params):
    BANK_SETTINGS_PREFIXES = ('M4 SoftPOS', 'РСБ СБП - QR')
    return not device_bank_params.startswith(BANK_SETTINGS_PREFIXES)
...
class BackOfficeKateMedia:
    ...
    def open_page(self):
        ...
        try:
            self.driver.get(BACKOFFICE_URL)
    ...
    def edit_all_devices_on_location(self):
        ...
        for i in range(0, 12):
            ...
            device_bank_params_block = device.find_element(By.CLASS_NAME,
                                                           '_3VxXWuyGnN7v40JTpbs5et._1vlczrDjSOp88EFyd3oT0u._1bLCZv_vwkVv53Jpeb0Ozx')
            device_bank_params = device_bank_params_block.find_element(By.CLASS_NAME, '_14JEb8M6MGYOa4D3Lq3_NR').text

            def add_bank_settings():
                ...
                hardware_id = hardware_id_block.find_element(By.CSS_SELECTOR,
                                                             'input[data-test="HardwareIdFieldInput"]').get_attribute(
                    'value')
                ...
                empty_settings_input = single_bank_settings_block.find_element(By.CLASS_NAME,
            ...
            if not is_device_tablet(device_name):
                continue
            elif not is_device_have_bank_settings(device_bank_params):
                continue
            else:
                add_bank_settings()
